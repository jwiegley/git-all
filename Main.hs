...
findDirsContainingW c curDepth p =
    ...
    (findDirsContaining c curDepth p)
...
findDirsContaining c curDepth p = do
    ...
    when (isRegularFile status) $ do
      ...
      when ("gitdir: " `L.isPrefixOf` t) $
        writeChan
          c
          ( Just
              ( takeDirectory p </> fromText (T.strip (T.pack (L.drop 8 t)))
              , takeDirectory p
              )
          )
  ...
  when
    ( isDirectory status
        && takeFileName p `notElem` ["CVS", ".svn", ".deps", "_darcs", "CMakeFiles"]
    )
    $ do
